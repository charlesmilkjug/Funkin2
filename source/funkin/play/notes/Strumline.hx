...
class Strumline extends FlxSpriteGroup
{
  ...
  public function vwooshInNotes():Void
  {
    ...
    for (note in notes.members)
    {
      ...
      note.yOffset = 200;
      if (Preferences.downscroll) note.yOffset = -200;
    ...
    for (holdNote in holdNotes.members)
    {
      ...
      if (Preferences.downscroll) holdNote.yOffset = -200;
  ...
  public function updateNotes():Void
  {
    ...
    for (holdNote in holdNotes.members)
    {
      ...
      else if (holdNote.missedNote && (holdNote.fullSustainLength > holdNote.sustainLength))
      {
        ...
        {
          if (Preferences.downscroll) holdNote.y = this.y - INITIAL_OFFSET + calculateNoteYPos(holdNote.strumTime + yOffset) - holdNote.height
            + STRUMLINE_SIZE / 2;
          else
