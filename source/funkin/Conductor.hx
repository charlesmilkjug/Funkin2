...
class Conductor
{
  ...
  {
    if (timeChanges.length == 0) return new SongTimeChange(0, 100);
    var i:Int = 0;
    ms = ms < 0 ? 0 : ms;
    for (timeChange in timeChanges)
    {
      i++;
      if ((i == timeChanges.length && ms >= timeChange.timeStamp)
        || (ms >= timeChange.timeStamp && ms < timeChanges[i].timeStamp)) return timeChange;
    }
