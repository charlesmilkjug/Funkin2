...
class FreeplayState extends MusicBeatSubState
{
  ...
  override function create():Void
  {
    ...
    if (PlayerRegistry.instance.countUnlockedCharacters() > 1) add(charSelectHint);
    ...
    var onDJIntroDone = () -> {
      ...
      if (prepForNewRank && fromResultsParams != null) rankAnimStart(fromResultsParams, grpCapsules.members[curSelected]);
    };
  ...
  function rankAnimStart(fromResults:FromResultsParams, capsuleToRank:SongMenuItem):Void
  {
    ...
    if (fromResultsParams?.newRank == SHIT) dj?.fistPumpLossIntro();
    else
      dj?.fistPumpIntro();
  ...
  function rankDisplayNew(fromResults:Null<FromResultsParams>, capsuleToRank:SongMenuItem):Void
  {
    ...
    new FlxTimer().start(0.6, _ -> rankAnimSlam(fromResults, capsuleToRank));
  }
  ...
  function rankAnimSlam(fromResultsParams:Null<FromResultsParams>, capsuleToRank:SongMenuItem):Void
  {
    ...
    new FlxTimer().start(0.5, _ -> {
      ...
      if (fromResultsParams?.newRank == SHIT) if (dj != null) dj.fistPumpLoss();
      else if (dj != null) dj.fistPump();
        ...
        if (distFromSelected < 5)
        {
          ...
          if (index > curSelected)
          {
            ...
            new FlxTimer().start(distFromSelected / 20, _ -> {
              ...
              IntervalShake.shake(capsule, 0.6, 1 / 24, 0.12 / (distFromSelected + 1), 0, FlxEase.quadOut, (_) -> capsule.doLerp = true);
            });
          }
  ...
  function handleInputs(elapsed:Float):Void
  {
    ...
    if (FlxG.onMobile)
    {
      ...
      {
        if (touch.justPressed) initTouchPos.set(touch.viewX, touch.viewY);
  ...
  function openDeleteSongDataOption(cap:SongMenuItem, targetSongId:String):Void
  {
    ...
    capsuleOptionsMenu.onConfirm = (targetOption:String) -> {
  ...
  function openInstrumentalList(cap:SongMenuItem, instrumentalIds:Array<String>):Void
  {
    ...
    capsuleOptionsMenu.onConfirm = (targetInstId:String) -> capsuleOnConfirmDefault(cap, targetInstId);
  }
  ...
  function refreshCapsuleDisplays():Void
    grpCapsules.forEachAlive((cap:SongMenuItem) -> cap.refreshDisplay());
