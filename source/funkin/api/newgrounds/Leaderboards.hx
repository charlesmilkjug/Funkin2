...
enum abstract Leaderboard(Int) from Int to Int
{
  ...
  public static function getLeaderboardBySong(songId:String, difficulty:String):Leaderboard
  {
    ...
    if (difficultyParts.length >= 2) variation = difficultyParts[difficultyParts.length - 1];
    else if (Constants.DEFAULT_DIFFICULTY_LIST_ERECT.contains(difficulty)) variation = "erect";
