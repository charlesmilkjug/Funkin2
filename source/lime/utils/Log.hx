...
class Log
{
  ...
  public static function verbose(message:Dynamic, ?info:PosInfos):Void
    if (level >= LogLevel.VERBOSE) println("[" + info.className + "] " + message);
  ...
  private static function __init__():Void
  {
    ...
    #if sys
    var args = Sys.args();
    if (args.indexOf("-v") > -1 || args.indexOf("-verbose") > -1) level = VERBOSE;
    else
    #end
    {
      #if debug
    ...
    if (untyped #if haxe4 js.Syntax.code #else __js__ #end ("console").log == null) untyped #if haxe4 js.Syntax.code #else __js__ #end ("console").log = () ->
      {};
    #end
  }
}
